...
class GoogleMapsScraper:
    ...
    def safe_get_text(
        self, selector: str, timeout: int = 2000, default: str = "UNKNOWN"
    ) -> str:
    ...
    def extract_place_info(self) -> Place:
        ...
        try:
            ...
            if target_button:
                ...
                for i in range(7):
                    # We need to click the button to open menu each time if it closes,
                    # or checking if menu is open.
                    # Usually clicking an item closes the menu?
                    # The original logic clicked the button every time.
                    ...
                    for j in range(count):
                        ...
                        if busy_hour.hour not in hours:
                            ...
                            day = (
                                self.page.locator("span[class='uEubGf NlVald']")
                                .nth(6)
                                .inner_text()
                            )
        ...
        try:
            ...
            reviews_node = self.page.locator(
                "div.F7nice span[aria-label*='reviews'], div.F7nice span[aria-label*='bài đánh giá']"
            ).first
            if reviews_node.count() > 0:
                ...
                reviews_btn = (
                    self.page.locator("button[jsaction*='review']")
        ...
        print(
            f"[extract] Done: {place.title} | stars={place.stars} | reviews={place.num_reviews} | lat={place.latitude} | lng={place.longitude} | about_keys={list(place.about.keys()) if isinstance(place.about, dict) else 'N/A'}"
        )
        return place
    ...
    def extract_about(self) -> Dict:
        ...
        try:
            ...
            for t in range(tab_count):
                ...
                if (
                    tab_text.lower() in ("about", "giới thiệu")
                    or "about" in tab_label.lower()
                    or "giới thiệu" in tab_label.lower()
                ):
                    about_node = tab
                    break
            ...
            if about_node:
                ...
                for i in range(sections_count):
                    ...
                    if header.count() > 0:
                        section_key = (
                            header.inner_text().strip().lower().replace(" ", "_")
                        )
                        items = []
    ...
    def extract_reviews(self) -> List[Dict]:
        ...
        try:
            ...
            for i in range(tab_count):
                ...
                if (
                    "Reviews" in text
                    or "Bài đánh giá" in text
                    or "Reviews" in aria
                    or "Bài đánh giá" in aria
                ):
                    review_tab = tab
                    break
            ...
            try:
                ...
                try:
                    ...
                    print(
                        "[extract_reviews] No review cards appeared after clicking tab."
                    )
                ...
                for i in range(10):  # Check up to 10 levels up
                    ...
                    try:
                        ...
                        overflow_y = parent.evaluate(
                            "el => window.getComputedStyle(el).overflowY"
                        )

                        if scroll_height > client_height and (
                            overflow_y == "auto"
                            or overflow_y == "scroll"
                            or overflow_y == "visible"
                        ):
            ...
            if scrollable_div and scrollable_div.count() > 0:
                ...
                for i in range(REVIEW_SCROLLING):
                    ...
                    try:
                        # Get current height
                        current_height = scrollable_div.evaluate(
                            "el => el.scrollHeight"
                        )
                        ...
                        if new_height == current_height:
                            ...
                            if no_change_count >= 3:
                                print(
                                    "Reached end of reviews (content height unchanged)."
                                )
                                break
                        else:
                            no_change_count = 0
            ...
            for i in range(count):
                try:
                    ...
                    try:
                        ...
                        review.user_name = user_info.locator("div.d4r55").inner_text(
                            timeout=1000
                        )
                        review.user_level = user_info.locator("div.RfnDt").inner_text(
                            timeout=1000
                        )
                    except Exception:
                    ...
                    # Rating
                    try:
                        star_label = review_card.locator(
                            "span.kvMYJc[role='img']"
                        ).get_attribute("aria-label", timeout=1000)
                        if star_label:
                    ...
                    # Time
                    try:
                        time_text = review_card.locator("span.rsqaWe").inner_text(
                            timeout=1000
                        )
                        review.time_review = (
                            time_text.strip()
                    ...
                    try:
                        ...
                        if more_button.count() > 0:
                            ...
                            self.page.wait_for_timeout(2000)
                        content_text = review_card.locator("span.wiI7pd").inner_text(
                            timeout=1000
                        )
                        review.content = (
                            content_text.strip()
                    ...
                    try:
                        visited_on = review_card.locator("div.PBK6be").first.inner_text(
                            timeout=1000
                        )
                        wait_time = (
                            review_card.locator("div.PBK6be")
                            .nth(1)
                        ...
                        review.visited_on = (
                            visited_on.strip()
                            if visited_on and visited_on.strip()
                            else "UNKNOWN"
                        )
                        review.wait_time = (
                            wait_time.strip()
                            if wait_time and wait_time.strip()
                            else "UNKNOWN"
                        )
                        review.reservation_recommended = (
                            reservation_recommended.strip()
                            if reservation_recommended
                            and reservation_recommended.strip()
                            else "UNKNOWN"
                        )
                    except Exception:
        ...
        with sync_playwright() as p:
            ...
            try:
                ...
                if wait_for_login:
                    print("---------------------------------------------------------")
                    print(
                        "If you are not logged in, please log in now in the browser window."
                    )
                    print(
                ...
                for q_idx, search_query in enumerate(search_queries):
                    ...
                    try:
                        ...
                        # Not all queries will show "Things to do", sometimes it shows places directly
                        # If "Things to do" is not found, try to find results list directly
                        try:
                            ...
                            if things_btn.count() > 0 and things_btn.first.is_visible():
                                ...
                                print(
                                    "Things to do button not found or not visible, assuming direct results or different layout."
                                )
                        except Exception as e:
                        ...
                        if feed.count() > 0:
                            ...
                            print(
                                "Feed container not found, maybe single result or different layout."
                            )
                            ...
                            try:
                                ...
                                if (
                                    ...
                                    if coord in seen_coords:
                                        print(
                                            f"Skipping duplicate place: {place.title} at {coord}"
                                        )
                                        continue
                                ...
                                # results.append(asdict(Place())) # Skip adding empty places to avoid noise
                                continue

                        if limit and len(results) >= limit:
                            break
                        # Go back to home for next query or just clear search box
                        # self.page.goto("https://www.google.com/maps", timeout=60000)
                        # self.page.wait_for_timeout(2000)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Scrape Google Maps Food")
    parser.add_argument(
        "--limit", type=int, default=None, help="Limit number of places to scrape"
    )
    args = parser.parse_args()

    # Use a directory for storing user profile (this will create it if not exists)
    user_data_dir = "google_maps_user_data_food"

    # Regions to scrape
    regions = ["huyện Hoàng Su Phì", "huyện Xín Mần", "huyện Bắc Quang", "huyện Quang Bình"]

    queries = []
    for region in regions:
        queries.append(f"nhà hàng {region}, Hà Giang")
        queries.append(f"quán ăn {region}, Hà Giang")
